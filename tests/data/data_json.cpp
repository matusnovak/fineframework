...
static bool compare(const ffw::Var& a, const ffw::Var& b, bool verbose) {
	...
	if (a.is<ffw::Object>() && b.is<ffw::Object>()) {
		auto ao = a.getAs<ffw::Object>();
		auto bo = b.getAs<ffw::Object>();
	...
	if (a.is<ffw::Array>() && b.is<ffw::Array>()) {
		auto ao = a.getAs<ffw::Array>();
		auto bo = b.getAs<ffw::Array>();
...
TEST_CASE("Decode JSON #1", "[JSON]") {
	...
	REQUIRE_NOTHROW(REQUIRE(json.getAs<ffw::Object>().size() == 7));
	...
	REQUIRE_NOTHROW(REQUIRE(json["object"].getAs<ffw::Object>().size() == 2));
	REQUIRE_NOTHROW(REQUIRE(json["object"]["first"].toInt() == 10));
	REQUIRE_NOTHROW(REQUIRE(json["object"]["second"].toInt() == 20));
	REQUIRE_NOTHROW(REQUIRE(json["array"].getAs<ffw::Array>().size() == 2));
...
TEST_CASE("Encode JSON #1", "[JSON]") {
	SECTION("Formated") {
		...
		ffw::Var test = ffw::decodeJson(str);
