#include "../source/data/tinyxml2.h"
#include <ffw/data.h>
...
TEST_CASE("Encode XML attributes #1", "[XML]") {
	...
	ffw::Var test = ffw::decodeXml(stream.get());

	REQUIRE(compare(test, example1, true));
}

TEST_CASE("Decode XML #1", "[XML]") {
	ffw::Var xml = ffw::decodeXml(
...
TEST_CASE("Encode XML #1", "[XML]") {
	SECTION("Formated") {
		...
		ffw::Var test = ffw::decodeXml(str);
...
TEST_CASE("Encode XML escaped character #1", "[JSON]") {
	ffw::Var xml = ffw::Object{ { "document",  ffw::Object() } };
