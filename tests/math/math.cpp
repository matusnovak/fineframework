...
TEST_CASE("Testing GetFileInfo", "[FileInfo]") {
	{
		...
		REQUIRE("C:/Hello/" == info.dirname);
		REQUIRE("World" == info.basename);
		REQUIRE("txt" == info.extension);
	}
...

	{
		...
		REQUIRE("C:/Hello/" == info.dirname);
		REQUIRE(".txt" == info.basename);
		REQUIRE("" == info.extension);
	}

	{
		ffw::FileInfo info = ffw::GetFileInfo("C:");
		REQUIRE("C:" == info.dirname);
		REQUIRE("" == info.basename);
		REQUIRE("" == info.extension);
	}
	...
	{
		ffw::FileInfo info = ffw::GetFileInfo("./Hello");
		REQUIRE("./" == info.dirname);
		REQUIRE("Hello" == info.basename);
		REQUIRE("" == info.extension);
	}
...

	{
		...
		REQUIRE("" == info.dirname);
		REQUIRE("image" == info.basename);
		REQUIRE("png" == info.extension);
	}
}
...
TEST_CASE("Testing Var", "[Var]") {
	...
	ffw::Var v;
	REQUIRE(v.Typeid() == typeid(std::nullptr_t));
	REQUIRE(v.Typeid() != typeid(int));
	REQUIRE(v.Empty());
	REQUIRE(v.Contains<std::nullptr_t>());

	ffw::Var a(123);
	REQUIRE(a.Typeid() == typeid(int));
	REQUIRE(a.Typeid() != typeid(float));
	REQUIRE(a.Empty() == false);
	REQUIRE(a.Contains<int>());

	REQUIRE_THROWS(static_cast<float>(a));

	REQUIRE_NOTHROW(static_cast<int>(a));
	...
	b = 11.22f;
	REQUIRE(b.Typeid() == typeid(float));
	REQUIRE(11.22f == (float)b);
	...
	b = std::string("hello");
	REQUIRE(b.Typeid() == typeid(std::string));
	REQUIRE(b == "hello");
	REQUIRE(b == std::string("hello"));

	ffw::Var c = b;
	REQUIRE(c.Typeid() == typeid(std::string));
	REQUIRE(std::string("hello") == (std::string)c);

	REQUIRE(c == "hello");
	REQUIRE(c == (std::string)b);
	REQUIRE(c != 42);
	...
	//REQUIRE(ffw::Var(false) == false);

	ffw::Var t = false;
	REQUIRE(t.Typeid() == typeid(bool));
...
TEST_CASE("Testing Var Object", "[VarObject]") {
	...
	try {
		...
		REQUIRE(first.Typeid() == typeid(int));
		REQUIRE(second.Typeid() == typeid(float));
		REQUIRE(third.Typeid() == typeid(const char*));
		REQUIRE(fourth.Typeid() == typeid(nullptr));
		REQUIRE(fifth.Typeid() == typeid(bool));
		REQUIRE(sixth.Typeid() == typeid(double));
	...
	try {
		...
		REQUIRE(hello.Typeid() == typeid(int));
		REQUIRE(42 == (int)hello);

		const auto& who = big["second"]["who"];
		REQUIRE(who.Typeid() == typeid(const char*));

		std::string test = (const char*)who;

		REQUIRE(big["second"]["who"] == "doctor");
		REQUIRE(who == "doctor");
...
TEST_CASE("Testing Var Array", "[VarArray]") {
	...
	REQUIRE(a[0].Typeid() == typeid(const char*));
	REQUIRE(a[1].Typeid() == typeid(int));
	REQUIRE(a[2].Typeid() == typeid(float));
	REQUIRE(a[3].Typeid() == typeid(bool));
...
TEST_CASE("Testing string tokenizer", "[String Math]") {
	{
