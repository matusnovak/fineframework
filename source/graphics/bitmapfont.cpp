...
///=============================================================================
bool ffw::BitmapFont::createFromData(const void* pixels, int width, int height, ffw::ImageType format, int points, int dpi, int start, int end) {
    ...
    if (format == ffw::ImageType::GRAYSCALE_8) {
        alphaOnlyFlag = true;
        result = texture.create(width, height, GL_RED, GL_RED, GL_UNSIGNED_BYTE);
    }
    else if(format == ffw::ImageType::RGB_ALPHA_8888) {
        alphaOnlyFlag = false;
        result = texture.create(width, height, GL_RGBA8, GL_RGBA, GL_UNSIGNED_BYTE);
    }
    else {
...
///=============================================================================
void ffw::BitmapFont::setCharData(wchar_t chr, ffw::Font::Char& data) {
...
///=============================================================================
bool ffw::BitmapFont::createFromBuffer(const ffw::ImageBuffer& buffer, int points, int dpi, int start, int end) {
    return createFromData(buffer.getPtr(), buffer.getWidth(), buffer.getHeight(), buffer.getImageType(), points, dpi, start, end);
}
...
///=============================================================================
int ffw::BitmapFont::getCharIndex(wchar_t chr) const {
    // Is outside of the range?
    if (chr < offsetStart || chr > offsetEnd)return 0;
    // Is the character not rendered?
    if (characters[chr - offsetStart].x < 0)return 0;
    // Return actual character
