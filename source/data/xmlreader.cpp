...
static const char* getErrorStr(tinyxml2::XMLError err) {
	switch (err) {
		...
		case tinyxml2::XMLError::XML_ERROR_ELEMENT_MISMATCH: return "XML_ERROR_ELEMENT_MISMATCH";
		...
		case tinyxml2::XMLError::XML_ERROR_IDENTIFYING_TAG: return "XML_ERROR_IDENTIFYING_TAG";
...
ffw::XmlReader::XmlReader(const std::string& input) :document(new tinyxml2::XMLDocument()), child(NULL), parent(NULL) {
	...
	if (err != tinyxml2::XMLError::XML_NO_ERROR) {
		throw ffw::XmlDecodeException("Failed to parse document! Error: " + std::string(getErrorStr(err)) + " at: " + document->GetErrorStr1());
	}
}
...
static ffw::Var decodeXmlAttr(ffw::XmlReader::Attribute& val) {
	if (val.isEmpty())return ffw::Var();
	if (val.isNumber()) {
		auto str = val.rawCstr();
		if (strstr(str, ".") != NULL) {
			return ffw::Var(val.toFloat());
		}
		return ffw::Var(val.toInteger());
	}
	if (val.isBoolean()) return ffw::Var(val.toBoolean());
	if (val.isNull())return ffw::Var(nullptr);
	if (val.isString())return ffw::Var(val.toString());

	throw ffw::XmlDecodeException("Could not detect value type! At: " + val.name());
}

static ffw::Var decodeXmlObject(ffw::XmlReader& xml, ffw::XmlReader::Node& node, std::string* arrKey);

static ffw::Var decodeXmlElem(ffw::XmlReader& xml, ffw::XmlReader::Node& val) {
	if (val.isEmpty()) {
		...
		while (xml.getNext(&child)) {
			...
			if (out.has_key(child.name())) {
				...
				if (!at.is<ffw::Array>()) {
					...
					at = ffw::Array();
					at.getAs<ffw::Array>().push_back(temp);
				}

				at.getAs<ffw::Array>().push_back(decodeXmlElem(xml, child));
			}
			else {
...
ffw::Var ffw::decodeXml(const char* input) {
...
ffw::Var ffw::decodeXmlFile(const std::string& path) {
	return ffw::Var();
}