...
///=============================================================================
void encodeXmlArr(ffw::XmlWriter& xml, const ffw::Var& obj) {
	if (obj.is<ffw::Array>()) {
		for (const auto& elem : obj.getAs<ffw::Array>()) {
			xml.addObject("element");
			xml.beginContent();

			if (elem.is<ffw::Object>()) {
				encodeXmlObj(xml, elem.getAs<ffw::Object>());
			}
			else {
...
void encodeXmlObj(ffw::XmlWriter& xml, const ffw::Object& obj) {
	for (const auto& pair : obj) {
		if (pair.second.is<ffw::Object>()) {
			...
			xml.beginContent();
			encodeXmlObj(xml, pair.second.getAs<ffw::Object>());
			xml.stepOut();
		}

		else if (pair.second.is<ffw::Array>()) {
			for (const auto& elem : pair.second.getAs<ffw::Array>()) {
				xml.addObject(pair.first.c_str());
				xml.beginContent();
...
///=============================================================================
std::string ffw::encodeXml(const ffw::Var& input, const char* indent, bool escape) {
	ffw::XmlWriter::StringStream stream;
	ffw::XmlWriter xml(&stream, indent, escape);

	if (input.is<ffw::Object>()) {
		encodeXmlObj(xml, input.getAs<ffw::Object>());
	}
	else {
...
///=============================================================================
bool ffw::encodeXmlFile(const std::string& path, const ffw::Var& input, const char* indent, bool escape) {
