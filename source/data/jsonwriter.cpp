...
static void encodeJsonObj(ffw::JsonWriter& json, const ffw::Object& input) {
    for (const auto& pair : input) {
        if (pair.second.is<ffw::Object>()) {
            ...
            encodeJsonObj(json, pair.second.getAsObject());
            json.stepOut();
        }

        else if (pair.second.is<ffw::Array>()) {
            json.addArray(pair.first.c_str());
            encodeJsonArr(json, pair.second.getAsArray());
            json.stepOut();
        }
...
static void encodeJsonArr(ffw::JsonWriter& json, const ffw::Array& input) {
    for (const auto& element : input) {
        if (element.is<ffw::Object>()) {
            json.addObject(NULL);
            encodeJsonObj(json, element.getAsObject());
            json.stepOut();
        }

        else if (element.is<ffw::Array>()) {
            json.addArray(NULL);
            encodeJsonArr(json, element.getAsArray());
            json.stepOut();
        }
...
///=============================================================================
std::string ffw::encodeJson(const ffw::Any& input, const char* indent, bool escape) {
    ...
    if (input.is<ffw::Object>()) {
        ...
        encodeJsonObj(json, input.getAsObject());
    }
    else if (input.is<ffw::Array>()) {
        json.addArray();

        encodeJsonArr(json, input.getAsArray());
    }
    else {
...
///=============================================================================
bool ffw::encodeJsonFile(const std::string& path, const ffw::Any& input, const char* indent, bool escape) {
