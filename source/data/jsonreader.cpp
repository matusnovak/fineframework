...
static ffw::Any decodeJsonObject(ffw::JsonReader& json);

///=============================================================================
static ffw::Any decodeJsonValue(ffw::JsonReader& json, ffw::JsonReader::Node& node) {
    if (node.isEmpty())return ffw::Any();
    if (node.isString())return ffw::Any(node.toString());
    if (node.isNumber()) {
        for (size_t i = 0; i < node.len; i++) {
            if (node.ptr[i] == '.') {
                return ffw::Any(node.toFloat());
            }
        }
        return ffw::Any(node.toInteger());
    }
    if (node.isBoolean()) return ffw::Any(node.toBoolean());
    if (node.isNull())return ffw::Any(nullptr);
...
///=============================================================================
ffw::Any ffw::decodeJson(const char* input) {
...
///=============================================================================
ffw::Any ffw::decodeJson(const std::string& input) {
    return decodeJson(input.c_str());
}

///=============================================================================
ffw::Any ffw::decodeJsonFile(const std::string& path) {
