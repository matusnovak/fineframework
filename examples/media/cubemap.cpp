...
public:
    ...
    bool setup() override {
		...
		// Load shader
		try {
			monkeyShader.createFromData(this, "", monkeyVertexShaderCode, monkeyFragmentShaderCode);
		...
		try {
			skyboxShader.createFromData(this, "", skyboxVertexShaderCode, skyboxFragmentShaderCode);
		} catch(std::exception& e){
			// Error while loading shader
		...
		if(!skybox.create(this, 1024, 1024, GL_RGB8, GL_RGB, GL_UNSIGNED_BYTE)){
		...
		if(!ffw::readObj("monkey.obj", this, &monkeyVbo)){
			std::cerr << "Failed to load monkey.obj!" << std::endl;
			return false;
		}

		// Load skybox VBO
		if(!skyboxVbo.create(this, skyboxVertices, sizeof(skyboxVertices), GL_STATIC_DRAW)) {
