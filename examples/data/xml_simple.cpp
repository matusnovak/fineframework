...
int main() {
    ...
    try {
        ...
        assert(xml.getAsObject()["menu"].getAsObject()["id"].toString() == "file");
        ...
        ffw::Array& arr = xml["menu"]["popup"]["menuitem"].getAsArray();
        ...
        try {
            ...
            std::cout << xml.getAsObject().at("menu").getAsObject().at("invalid").getAs<std::string>() << std::endl;
